...
(defn custom-not-found-handler [request]
  (let [request (-> ((db/wrap-tx (fn [request] request)) request)
                    ((csrf/extract-header (fn [request] request)) request)
                    ((session/wrap-authenticate (fn [request] request)) request)
                    ((dm/extract-dev-cookie-params (fn [request] request)) request))
        uri (:uri request)
