...
feature "Create license", type: :feature do
  ...
  scenario "works" do
    ...
    click_on "Add allocation"

    fill_in "Quantity", with: total_quantity.to_i + 5
    expect(page).to have_content "Remaining -#{total_quantity.to_i - total_quantity.to_i + 5}"
    fill_in "Quantity", with: total_quantity
    expect(page).to have_content "Remaining 0"
    ...
    within page.find('#fields\.Maintenance\.title') do
      expect(page).not_to have_content "Maintenance expiration"
      expect(page).not_to have_content "Currency"
    ...
    click_on "Currency"
    within find('[data-test-id="properties_maintenance_currency-options"]') do
