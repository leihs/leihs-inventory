...
feature "Create model", type: :feature do
  ...
  scenario "works" do
    ...
    within id: "pool.model.images.title" do
      find("input[type='file']", visible: false).attach_file "./spec/data/#{image_name_1}"
      find("input[type='file']", visible: false).attach_file "./spec/data/#{image_name_2}"
      find("button[role='radio']", match: :first).click
    end

    within id: "pool.model.attachments.title" do
      find("input[type='file']", visible: false).attach_file "./spec/data/#{attachment_name_1}"
      find("input[type='file']", visible: false).attach_file "./spec/data/#{attachment_name_2}"
    end
    ...
    # expect(
    #   find("label", text: "this is a package").find(:xpath, "..").find("input", visible: false)
    # ).to be_checked
