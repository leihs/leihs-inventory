(ns leihs.inventory.client.components.form.instant-search
  (:require
   ["@/components/ui/popover" :refer [Popover PopoverAnchor PopoverContent PopoverTrigger]]
   ["@@/button" :refer [Button]]
    ...
    (uix/use-effect
     (fn []
       (when (= (.. js/document -activeElement -id)
         ...
         (if (or (< (count debounced-search) 2)
                 (= debounced-search ""))
           ...
           (let [fetch (fn []
                         ...
                         (-> http-client
                             (.get (str path (js/encodeURIComponent debounced-search)))
                             (.then (fn [res]
