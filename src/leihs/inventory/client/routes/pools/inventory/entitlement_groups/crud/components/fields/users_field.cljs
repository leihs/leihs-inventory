  ...
  (let [[t] (useTranslation)
        ...
        handle-select (fn [selected]
                        ...
                        (if (not (check-path-existing (:id selected) fields))
                          (append (cj (merge {:firstname (:firstname selected)
                                              :lastname (:lastname selected)
       ...
       (when (> (count debounced-search) 1)
         (let [fetch (fn []
                       ...
                       (-> http-client
                           ...
                           (.catch
                            (fn [err]
                              (js/console.error "Error fetching users" err)))))]
           (fetch))))
             ...
             ($ CommandList {:data-test-id "users-list"}
                ...
                (for [element data]
                  ($ CommandItem {:key (:id element)
                                  :value (user-label element props t)
