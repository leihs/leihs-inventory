    ...
    context "GET /inventory/:pool-id/items?page=1&filter_q= (EDN filter)" do
      ...
      describe "filtering by boolean fields" do
        describe "is_borrowable" do
          it "filters by is_borrowable {:$eq false}" do
            data = fetch_filtered("{:is_borrowable {:$eq false}}")
            ids = data.map { |i| i["id"] }
            expect(ids).to include(entry[:id].to_s)
            expect(ids).to include(entry_low_price[:id].to_s)
            expect(ids).not_to include(entry_borrowable[:id].to_s)
          ...
          it "filters by is_borrowable {:$eq true}" do
            data = fetch_filtered("{:is_borrowable {:$eq true}}")
            ids = data.map { |i| i["id"] }
            expect(ids).to include(entry_borrowable[:id].to_s)
            expect(ids).not_to include(entry[:id].to_s)
            expect(ids).not_to include(entry_low_price[:id].to_s)
        ...
        describe "is_broken" do
          it "filters by is_broken {:$eq false}" do
            data = fetch_filtered("{:is_broken {:$eq false}}")
            ids = data.map { |i| i["id"] }
            expect(ids).to include(entry[:id].to_s)
            expect(ids).to include(entry_low_price[:id].to_s)
            expect(ids).not_to include(entry_borrowable[:id].to_s)
          ...
          it "filters by is_broken {:$eq true}" do
            data = fetch_filtered("{:is_broken {:$eq true}}")
            ids = data.map { |i| i["id"] }
            expect(ids).to include(entry_borrowable[:id].to_s)
            expect(ids).not_to include(entry[:id].to_s)
          end
        ...
        describe "is_incomplete" do
          it "filters by is_incomplete {:$eq false}" do
            data = fetch_filtered("{:is_incomplete {:$eq false}}")
            ids = data.map { |i| i["id"] }
            expect(ids).to include(entry[:id].to_s)
            expect(ids).to include(entry_low_price[:id].to_s)
            expect(ids).not_to include(entry_borrowable[:id].to_s)
          ...
          it "filters by is_incomplete {:$eq true}" do
            data = fetch_filtered("{:is_incomplete {:$eq true}}")
            ids = data.map { |i| i["id"] }
            expect(ids).to include(entry_borrowable[:id].to_s)
            expect(ids).not_to include(entry[:id].to_s)
          end
      ...
      describe "comprehensive field and operator coverage" do
        ...
        describe "UUID fields" do
          ...
          it "parent_id (:$eq) — filters child items by parent" do
            ...
            data = fetch_filtered("{:parent_id {:$eq \"#{parent_item.id}\"}}")
            ids = data.map { |i| i["id"] }
            expect(ids).to include(child_item.id.to_s)
            expect(ids).not_to include(entry[:id].to_s)
            expect(ids).not_to include(entry_borrowable[:id].to_s)
          end
        end
        ...
        # === BOOLEAN FIELDS (explicit {:$eq true/false} required; scalar shorthand is rejected) ===
        describe "boolean fields" do
          ...
          it "is_borrowable scalar shorthand returns 400" do
            resp = client.get(filter_url("{:is_borrowable false}"))
            expect(resp.status).to eq(400)
          end
          ...
          it "is_broken scalar shorthand returns 400" do
            resp = client.get(filter_url("{:is_broken true}"))
            expect(resp.status).to eq(400)
          end
          ...
          it "is_inventory_relevant ({:$eq true})" do
            data = fetch_filtered("{:is_inventory_relevant {:$eq true}}")
            ids = data.map { |i| i["id"] }
          ...
          it "is_inventory_relevant ({:$eq false})" do
            data = fetch_filtered("{:is_inventory_relevant {:$eq false}}")
            ids = data.map { |i| i["id"] }
            expect(ids).to include(entry[:id].to_s)
            expect(ids).to include(entry_borrowable[:id].to_s)
            expect(ids).to include(entry_low_price[:id].to_s)
          ...
          it "needs_permission ({:$eq true})" do
            data = fetch_filtered("{:needs_permission {:$eq true}}")
            ids = data.map { |i| i["id"] }
          ...
          it "needs_permission ({:$eq false})" do
            data = fetch_filtered("{:needs_permission {:$eq false}}")
            ids = data.map { |i| i["id"] }
            expect(ids).to include(entry[:id].to_s)
            expect(ids).to include(entry_borrowable[:id].to_s)
          end

          # retired field: special nullable date column — boolean semantics unchanged,
          # scalar form remains valid for retired (it is not a boolean column).
          # Client sends true  → items where db.retired IS NOT NULL
          # Client sends false → items where db.retired IS NULL
          it "retired (scalar true) returns items with retirement date set" do
          ...
          it "retired (scalar false) returns items without retirement date" do
          ...
          it "retired (:$gte date) returns 400 — only boolean predicates allowed" do
            resp = client.get(filter_url('{:retired {:$gte "2024-01-01"}}'))
            expect(resp.status).to eq(400)
          end

          it "retired (:$lte date) returns 400 — only boolean predicates allowed" do
            resp = client.get(filter_url('{:retired {:$lte "2025-12-31"}}'))
            expect(resp.status).to eq(400)
          end

          it "retired (:$eq date string) returns 400 — only boolean predicates allowed" do
            resp = client.get(filter_url('{:retired {:$eq "2024-06-15"}}'))
            expect(resp.status).to eq(400)
          end

          it "retired (:$ilike string) returns 400 — only boolean predicates allowed" do
        ...
        describe "logical operators" do
          ...
          it ":$and with 3 conditions (boolean + numeric + string)" do
            data = fetch_filtered('{:$and [{:is_borrowable {:$eq false}} {:price {:$gte 100}} {:inventory_code {:$ilike "ITZ21122"}}]}')
            ids = data.map { |i| i["id"] }
            expect(ids).to include(entry[:id].to_s)
            expect(ids).not_to include(entry_borrowable[:id].to_s)
            expect(ids).not_to include(entry_low_price[:id].to_s)
          ...
          it ":$and where no items match all conditions" do
            data = fetch_filtered("{:$and [{:is_borrowable {:$eq true}} {:is_broken {:$eq false}}]}")
            ids = data.map { |i| i["id"] }
          ...
          it ":$or where only first condition matches" do
            data = fetch_filtered("{:$or [{:is_borrowable {:$eq true}} {:price {:$eq 999}}]}")
            ids = data.map { |i| i["id"] }
            expect(ids).to include(entry_borrowable[:id].to_s)
            expect(ids).not_to include(entry[:id].to_s)
            expect(ids).not_to include(entry_low_price[:id].to_s)
          ...
          it ":$or with nested :$and (2 subconditions each)" do
            ...
            data = fetch_filtered("{:$or [{:$and [{:is_borrowable {:$eq false}} {:price {:$gte 100}}]} {:$and [{:is_broken {:$eq true}} {:price {:$gte 200}}]}]}")
            ids = data.map { |i| i["id"] }
          ...
          it ":$or with nested :$and (3 subconditions in one)" do
            ...
            data = fetch_filtered("{:$or [{:$and [{:is_borrowable {:$eq false}} {:is_broken {:$eq false}} {:price {:$eq 100}}]} {:is_borrowable {:$eq true}}]}")
            ids = data.map { |i| i["id"] }
          ...
          it ":$or with nested :$and where only one branch matches" do
            ...
            filter = "{:$or [" \
              "{:$and [{:is_borrowable {:$eq false}} {:price {:$gte 500}}]} " \
              "{:$and [{:is_borrowable {:$eq true}} {:is_broken {:$eq true}}]} " \
              "{:$and [{:user_name {:$ilike \"xyz\"}} {:is_incomplete {:$eq true}}]}" \
            "]}"
          ...
          it ":$or combining boolean, numeric, string, and date conditions" do
            ...
            filter = "{:$or [" \
              "{:$and [{:is_broken {:$eq true}} {:price {:$gte 200}}]} " \
              "{:$and [{:user_name {:$ilike \"itz\"}} {:is_borrowable {:$eq false}} {:invoice_date {:$gte \"2013-09-01\"}}]} " \
          ...
          it ":$or with mixed field types (boolean, numeric, string)" do
            ...
            data = fetch_filtered('{:$or [{:is_borrowable {:$eq true}} {:price {:$lte 50}} {:inventory_code {:$ilike "ITZ21122"}}]}')
            ids = data.map { |i| i["id"] }
